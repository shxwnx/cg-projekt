...
void Model::calcBoundingBox(const aiScene* pScene, AABB& Box)
{
	...
	for (int meshIdx = 0; meshIdx < pScene->mNumMeshes; meshIdx++) {					//Durchgehen aller Meshes
		for (int vertIdx = 0; vertIdx < pScene->mMeshes[meshIdx]->mNumVertices; vertIdx++) {
																						//Durchgehen aller Vertizes des Meshes und Ermitteln der größten/kleinsten Werte
			float x = pScene->mMeshes[meshIdx]->mVertices[vertIdx].x;
			float y = pScene->mMeshes[meshIdx]->mVertices[vertIdx].y;
			float z = pScene->mMeshes[meshIdx]->mVertices[vertIdx].z;
